...
1. Griffiths & Harris goes about defining the curvature matrix of a connection $D$ on a complex vector bundle $E\to M$ as follows. For sections $\psi\in\mathcal{A}^p(U)$ and $\xi\in\mathcal{A}^0(E)(U)$, we enforce the Leibniz rule
\\[D(\psi\wedge\xi)=d\psi\otimes\xi+(-1)^p\psi\wedge D\zeta.\\]
Then a calculation shows that the operator $D^2\colon\mathcal{A}^0(E)\to\mathcal{A}^2(E)$ is $C^{\infty}$-linear: if $\sigma$ is a section of $E$ and $f$ is a $C^{\infty}$-function, we will have $D^2(f\cdot\sigma)=f D^2\sigma$. From here onward, I don't understand what follows. Griffiths & Harris states that it follows from this that $D^2\colon\mathcal{A}^0(E)\to\mathcal{A}^2(E)$ is induced by a bundle map $E\to\bigwedge^2T^{\*}\otimes E$, or in other words, $D^2$ corresponds to a global section $\Theta$ of the bundle
